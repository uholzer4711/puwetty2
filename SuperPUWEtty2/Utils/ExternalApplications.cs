...
namespace SuperPUWEtty2.Utils
{
    public static class ExternalApplications
    {
        ...
        {
            if (!String.IsNullOrEmpty(session.Password) && !SuperPUWEtty2.Settings.AllowPlainTextPuttyPasswordArg)
            ...
            String userPw =    !String.IsNullOrEmpty(user) ? (!String.IsNullOrEmpty(pw) && SuperPUWEtty2.Settings.AllowPlainTextPuttyPasswordArg ? user + ":" + pw      + "@" : user + "@") : "";
            String userPwLog = !String.IsNullOrEmpty(user) ? (!String.IsNullOrEmpty(pw) && SuperPUWEtty2.Settings.AllowPlainTextPuttyPasswordArg ? user + ":" + "XXXXX" + "@" : user + "@") : "";
            ...
            Log.Debug("Send to FileZilla:" + SuperPUWEtty2.Settings.FileZillaExe + " params="+ "sftp://" + userPwLog + session.Host + ":" + session.Port + rp + lp);
            Process.Start(SuperPUWEtty2.Settings.FileZillaExe, param);
        }
        ...
        public static void openWinSCP(SessionData session)
        {
            ...
            String param = "sftp://" + userPw + session.Host + ":" + session.Port + rp + lp;
            Log.Debug("Send to WinSCP:" + SuperPUWEtty2.Settings.WinSCPExe + " params="+ "sftp://" + userPwLog + session.Host + ":" + session.Port + rp + lp);
            Process.Start(SuperPUWEtty2.Settings.WinSCPExe, param);
        }
